  ...
  //  if (!(z_id >=0 && z_id < 3)) printf("Z is not in the assumed range\n");
   ...
   // if (gridSize==13) printf("x_id: %d, y_d: %d\n", x_id, y_id);
    ...
    output[bbindex + numGridCells * (z_id * (5 + numOutputClasses) + 1)]
        = sigmoidGPU(input[bbindex + numGridCells * (z_id * (5 + numOutputClasses) + 1)]); // bbox y

    output[bbindex + numGridCells * (z_id * (5 + numOutputClasses) + 2)]
        = __expf(input[bbindex + numGridCells * (z_id * (5 + numOutputClasses) + 2)]); // bbox w

    output[bbindex + numGridCells * (z_id * (5 + numOutputClasses) + 3)]
        = __expf(input[bbindex + numGridCells * (z_id * (5 + numOutputClasses) + 3)]); // bbox h

    output[bbindex + numGridCells * (z_id * (5 + numOutputClasses) + 4)]
        = sigmoidGPU(input[bbindex + numGridCells * (z_id * (5 + numOutputClasses) + 4)]); // Pr(object)

    for (uint i = 0; i < numOutputClasses; ++i) // Pr(class | object)
    {
