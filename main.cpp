...
struct Placemark
{
    ...
    float latitude, longditude, speed;
...
void post_process(Placemark& mark)
{
...
int main()
{
	...
	camera >> mark.original;
	mark.info = pre_process(mark.original);
	infer(context, mark.info);
	post_process(mark);
	auto t1 = std::chrono::system_clock::now();
	total_time += std::chrono::duration_cast<std::chrono::milliseconds>(t1 - t0).count();
	cv::imshow("Detections", mark.processed);
